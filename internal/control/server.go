...
func Start(cfg Config) *http.Server {
	...
	mux.HandleFunc("/cache/clear", handleCacheClear(cfg.Resolver, token))
	...
	mux.HandleFunc("/upstreams/reload", handleUpstreamsReload(cfg.Resolver, cfg.ConfigPath, token))
	mux.HandleFunc("/response/reload", handleResponseReload(cfg.Resolver, cfg.ConfigPath, token))
	mux.HandleFunc("/safe-search/reload", handleSafeSearchReload(cfg.Resolver, cfg.ConfigPath, token))
	mux.HandleFunc("/client-identification/reload", handleClientIdentificationReload(cfg.Resolver, cfg.ConfigPath, token))
