...
|----------------|--------|-------|
| `cache.refresh` | `enabled`, `hit_window`, `hot_threshold`, `min_ttl`, `hot_ttl`, `serve_stale`, `stale_ttl`, `lock_ttl`, `max_inflight`, `sweep_interval`, `sweep_window`, `max_batch_size`, `sweep_min_hits`, `sweep_hit_window` | Refresh sweeper tuning |
