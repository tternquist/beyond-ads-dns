...
- **Graceful degradation:** Stale serving, SERVFAIL backoff, upstream failover, connection pool retry on EOF; Redis degraded mode (L0-only when Redis unavailable, with automatic fallback and reconnect, configurable via `cache.redis.degraded_on_unavailable` or `REDIS_DEGRADED_ON_UNAVAILABLE`)
