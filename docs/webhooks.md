...
| `release_tag` | Release identifier for the running Metrics UI build. Resolution: `RELEASE_TAG` env → `release-tag.txt` file next to the build artifacts (when running in the official container). `null` when not available (e.g. local dev build). |
...
| `refresh_stats` | Sweeper stats: sweeps per 24h, refreshed/removed counts, batch size, sweep hit window, sweep min hits, entries removed (removed_24h). Requires DNS control URL. |
...
5. **Host IP shows container IP in Docker:** The app tries (1) `host.docker.internal` (via `extra_hosts` or Docker Desktop), then (2) the default gateway from `/proc/net/route` (Linux only—this works even when `extra_hosts` fails). If you still get the container IP or need your LAN IP (e.g. 192.168.1.10), set `HOST_IP` in `.env` or your compose environment.
