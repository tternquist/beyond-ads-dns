...
**Max table size (tmpfs):** ClickHouse has no built-in max database size. When using tmpfs, set `query_store.max_size_mb` (e.g. 200 for a 256MB tmpfs) or the `QUERY_STORE_MAX_SIZE_MB` env variable so the app drops oldest partitions when over the limit—preventing OOM.
